<x-admin-layout>
    ...
    <div class="row mb-5">
        <div class="col-md-6">
          <h4>Add a new Cashapp</h4>
          <form action="/manage-cashapp" method="POST" class="form">
            @csrf
            <!-- Price -->
            ...
            <button type="submit" class="btn btn-primary mt-2">Add Cashapp</button>
          </form>
          <!--  -->
        </div>
            ...
            <table class="table table-striped table-sm">
              ...
              <tbody>
                ...
                <tr>
                    <td>{{ $number++ }}</td>
                    <td>{{ 'Cashapp' }}</td>
                    <td>{{ $cashapp->description }}</td>
                    <td>{{ $cashapp->price }}</td>
                    <td>{{ $cashapp->balance }}</td>
                    <td>
                      <div class="d-flex">
                        <a href="/cashapp/{{ $cashapp->id }}/edit" class="btn btn-sm btn-outline-warning">Edit</a>
                        <a href="#" class="btn btn-sm btn-outline-info">Preview</a>
                        <form action="/cashapp/{{ $cashapp->id }}" method="POST">
                          @csrf
                          @method('DELETE')
